...
const PontoVenda = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedProdutos.length === 0) { toast.error("Adicione ao menos um celular"); return; }
    ...
    try {
      await axios.post(`${API}/loja/${lojaSlug}/vendas`, {
        ...
        troca: possuiTroca ? {
          ...
          memoria_ram: trocaMemoriaRam.trim() || null,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <Card className="bg-[#141414] border border-white/5">
            <CardHeader className="border-b border-white/5 py-4"><div className="flex items-center gap-3"><Package className="w-5 h-5 text-blue-500" /><CardTitle className="text-base font-semibold text-white">Celulares Disponíveis</CardTitle></div></CardHeader>
            <CardContent className="p-4">
              <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" /><Input placeholder="Buscar celular..." value={searchProduto} onChange={(e) => setSearchProduto(e.target.value)} className="pl-10 bg-[#0A0A0A] border-white/10 text-white" data-testid="search-produto-venda" /></div>
              <div className="max-h-[400px] overflow-y-auto space-y-2">
                ...
                )) : <p className="text-center text-gray-500 py-8">Nenhum celular disponível</p>}
              </div>
            </CardContent>
          </Card>
        </div>
              ...
              <div className="space-y-3 rounded-lg border border-white/10 p-3 bg-[#101010]">
                ...
                {possuiTroca && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-2"><Label className="text-gray-300">Memória RAM</Label><Input value={trocaMemoriaRam} onChange={(e) => setTrocaMemoriaRam(e.target.value)} placeholder="Ex: 8GB (opcional)" className="bg-[#0A0A0A] border-white/10 text-white" data-testid="input-troca-memoria-ram" /></div>
                    ...
                    <p className="text-xs text-gray-400 md:col-span-2">O celular recebido será cadastrado automaticamente no estoque e o valor informado será aplicado como desconto da venda.</p>
                  </div>
                )}
              </div>
